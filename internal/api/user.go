...
func (a *API) UserUpdate(w http.ResponseWriter, r *http.Request) error {
	...
	err := db.Transaction(func(tx *storage.Connection) error {
		...
		if params.Password != nil {
			...
			if terr := models.NewAuditLogEntry(config.AuditLog, r, tx, user, models.UserUpdatePasswordAction, "", nil, models.WithSession(session)); terr != nil {
				return terr
			}
		...
		if terr = models.NewAuditLogEntry(config.AuditLog, r, tx, user, models.UserModifiedAction, "", nil, models.WithSession(session)); terr != nil {
